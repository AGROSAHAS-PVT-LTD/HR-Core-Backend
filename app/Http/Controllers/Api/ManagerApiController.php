...
class ManagerApiController extends Controller
{
    ...
    public function addNewUserDataOld(Request $request)
    {
        ...
        $createdUser->phone_number = $validatedUserData['phone_number'];
        ...
        // Create a Default Team and Default Shift
        // Create default Team if it doesn't exist
        Team::firstOrCreate(
            ['name' => 'Default Team', 'business_id' => $businessId], // Search conditions
            [
                'description' => 'This is the default team.',
                'status' => 'active',
                'is_chat_enabled' => true,
                'created_by_id' => $createdUser->id, // Fallback to ID 1 if not authenticated
        ...
        Shift::firstOrCreate(
            ['title' => 'Default Shift', 'business_id' => $businessId], // Search conditions
            [
    ...
    public function addNewAccountDataOld(Request $request)
    {
        ...
        try {
            ...
            $business = Business::create([
                'start_date' => now(),
                'contact' => $validatedData['phone_number'],
            ...
            // reate and Attach Admin Role
            $adminRole = Role::create([
                'name' => 'Admin',
                'slug' => 'admin-' . $business->id,
                'permissions' => json_encode(['manage_all' => true]),
                'business_id' => $business->id,
                'description' => 'This role allows users to manage their team and view reports.',
            ]);
            ...
            $business->package_id = $validatedData['package'];
            $business->save();
            ...
            $owner->user_name = $business->id . '_' . Str::slug($validatedData['first_name'] . $validatedData['last_name']);
            $owner->save();
    
            // Create Subscription
            $package = Package::findOrFail($validatedData['package']);
            
            $price = $package->price;
            if ($validatedData['plan_type'] === 'yearly') {
            ...
            Subscription::create([
                ...
                'start_date' => now(),
                'end_date' => now()->add($package->interval_count, $package->interval),
                'business_id' => $business->id,
            ...
            // Send the email
            Mail::to($owner->email)->send(new BusinessCreatedMail($owner, $business, $validatedData['password']));
            DB::commit();
            return response()->json([
                'statusCode' => 200,
                'status' => 'success',
