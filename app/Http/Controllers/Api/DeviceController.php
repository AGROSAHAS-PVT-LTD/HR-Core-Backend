...
class DeviceController extends Controller
{
  public function checkDevice(Request $request)
  {
    ...
    // }
    $user =  auth()->user();
    if($user->email == "test@email.com" || $user->email == "employee@opencorehr.com"){
        $oldDevice = UserDevice::where('user_id', auth()->id())->first();
        ...
        $device->board = "test_board";
        $device->sdk_version = "222";
        $device->model = "test_model";
        $device->app_version = "3";
        $device->token = '';
        $device->latitude = 0;
        $device->longitude = 0;
        $device->save();
